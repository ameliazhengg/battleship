...
let check_valid_placement (row : int) (col : char) (dir : string) (length : int)
  ...
  else if dir = "right" && int_of_char col - 65 + length > 10 then
    false
...
let computer_2_ship () : ship= 
  let row = ref random_row in 
  let col = ref random_col in
  let dir = ref random_dir in
  let quit_loop = ref (check_valid_placement !row !col !dir 2) in
  while not !quit_loop do
    row := random_row;
    col := random_col;
    dir := random_dir;
    quit_loop := check_valid_placement !row !col !dir 2 
  done;
  {name = "2_ship"; length = 2; coordinates = (!row, !col); direction = !dir}
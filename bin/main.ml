open Final_proj_3110.Fleet
open Final_proj_3110.Coord

(* let one_one = new_coord 1 1 *)
(* let my_fleet = new_fleet let add_success fleet coord = string_of_bool (snd
   (add_ship fleet coord)) let does_contain fleet coord = string_of_bool
   (contains fleet coord) *)

let fleet1 = new_fleet ()
let fleet2 = fst (add_ship (new_fleet ()) (new_coord 2 2))

let prompt_and_print () =
  let () = print_string "Please enter your name: " in
  let the_input = read_line () in
  print_endline
    ("Welcome to Battleship, " ^ the_input ^ to_string fleet1 ^ "|||"
   ^ to_string fleet2)
